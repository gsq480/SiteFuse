...
<main id="main">
  ...
  <section id="services" class="py-20" style="background: white;">
    <div class="container">
      ...
      <!-- DEBUG START -->
      <?php
      $json_path = get_template_directory() . "/../../website.json";
      echo "<!-- DEBUG: Looking for website.json at: $json_path -->";
      echo "<!-- DEBUG: File exists: " . (file_exists($json_path) ? "YES" : "NO") . " -->";
      if (file_exists($json_path)) {
          $content = file_get_contents($json_path);
          echo "<!-- DEBUG: File size: " . strlen($content) . " bytes -->";
          $website_data = json_decode($content, true);
          echo "<!-- DEBUG: JSON decode success: " . ($website_data ? "YES" : "NO") . " -->";
          if ($website_data && isset($website_data["services"])) {
              echo "<!-- DEBUG: Found " . count($website_data["services"]) . " services -->";
              echo "<!-- DEBUG: First service: " . $website_data["services"][0]["title"] . " -->";
          }
      } else {
          echo "<!-- DEBUG: website.json not found -->";
      }
      ?>
      <!-- DEBUG END -->
